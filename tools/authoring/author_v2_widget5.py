...
HILITE = "/Game/UI/Materials/General/M_UI_FLX_InputHighlight.M_UI_FLX_InputHighlight"
...
def hcell(hbox_pin, widget_pin, ratio, halign, tail, pad=None):
    ...
    h = call(ed, "/Script/UMG.HorizontalBoxSlot:SetHorizontalAlignment", "cell halign"); connect(sl, selfpin(h), "slot"); setval(h, "InHorizontalAlignment", halign); connect(tail, exec_in(h), "-> halign"); tail = then_out(h)
...
for f_, v_ in (("SetHorizontalAlignment", "Center"), ("SetVerticalAlignment", "Center")):
    s = call(ed, f"/Script/UMG.OverlaySlot:{f_}", f_); connect(out(ao, "ReturnValue"), selfpin(s), "slot"); PL.set_pin_value(first_param(s), v_); connect(tail, exec_in(s), f"-> {f_}"); tail = then_out(s)
...
def vadd(vb, wp, tail, pad=None, halign=None):
    ...
    if halign:
        h_ = call(ed, "/Script/UMG.VerticalBoxSlot:SetHorizontalAlignment", "halign"); connect(out(a, "ReturnValue"), selfpin(h_), "slot"); PL.set_pin_value(first_param(h_), halign); connect(tail, exec_in(h_), "-> halign"); tail = then_out(h_)
    return tail
...
bh, tail = construct("/Script/UMG.HorizontalBox", "modal buttons", tail); tail = vadd(mv, bh, tail, halign="Center")
btns = {}
