"""v2 - Roster / Ranks tabs on the clan window. Run WITH -ModDevKit. GA_MODE=dry|real. GA_TARGET=player|guildview.
...
if TARGET == "player":
    ...
    for nm, t in (("RanksHost", lib.get_object_reference_type(unreal.GuildViewBase)), ("RanksPanel", lib.get_object_reference_type(unreal.load_class(None, W_C)))):
    ...
    victims = walk_exec(then_out(cw), stop_titles=("AddToViewport",)); atv = [n for n in ed.list_all_nodes() if title(n).replace(" ", "") == "AddToViewport"]; check(len(atv) == 1, "AddToViewport node"); atv = atv[0]
    ...
    br = ed.add_branch_node(); connect(out(iv, "ReturnValue"), lib.find_condition_pin(br), "valid?"); connect(then_out(sh), exec_in(br), "-> Branch"); connect(lib.find_else_pin(br), exec_in(atv), "no host -> AddToViewport (fallback)")
    ...
    tabs = {}
    for key, label in (("TabRoster", "Roster"), ("TabRanks", "Ranks")):
    ...
    sv = call(ed, f"{W_C}:SaveNow", "panel.SaveNow"); connect(p, selfpin(sv), "panel"); connect(then_out(b), exec_in(sv), "-> SaveNow"); t = then_out(sv)
    ...
    sbx, swp = roster_bits(); t = set_vis(ed, sbx, "Visible", t, "roster"); t = set_vis(ed, swp, "Visible", t, "show offline"); toggle(t, True)
